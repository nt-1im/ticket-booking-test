...
public interface TrainRepository extends JpaRepository<Train, Long> {
        ...
        @Query("SELECT t FROM Train t WHERE " +
                        "(:departure IS NULL OR :departure = '' OR LOWER(t.departureStation) LIKE LOWER(CONCAT('%', :departure, '%'))) AND "
                        +
                        "(:arrival IS NULL OR :arrival = '' OR LOWER(t.arrivalStation) LIKE LOWER(CONCAT('%', :arrival, '%'))) AND "
                        +
